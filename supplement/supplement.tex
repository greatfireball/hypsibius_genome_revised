...
\captionof{figure}[Assembly Feature Comparisons]{A) Per-site coverage
  ...
  coverage of the trusted, most likley nuclear, genome. B) GC concent
  ...
  shows a typical diploid spectrum. D) Lenght distribution of
...
\captionof{figure}[Unknown Bacterial Genome]{Circular map of an
  ...
  drawn with CGView. Track 1 and 2 (blue) indicate GeneMark-S annoated
...
\begin{sidewaystable}
\centering
\captionof{table}{\label{tab:inputdata}Used data set including checksums for compressed and decompressed file content.}
...
The genome size was esitmated by running the standalone error
...
The per-site variablity was calculated by counting bases covering each
site of the two assembly. Each base that occurred at a given site with
...
Lenght of the intragenetic regions were directly extracted from the
...
All resulting data set were compared, tested and visualized using the
GNU R package `sm'.
